...
namespace UIComponents.Models.Models.FileExplorer
{
    ...
    public static class UICFileExplorerAddons
    {
        ...
        public static List<IUIComponent> ContextMenu(UICFileExplorer fileExplorer)
        {
            ...
            var createDirectory = new UICContextMenuItem($"#{fileExplorer.GetId()}:has(.can-create-folder)", new UICDropdownItem(TranslatableSaver.Save("FileExplorer.CreateDirectory")) { Icon = IconDefaults.CreateFolder?.Invoke() }, new UICCustom("uic.fileExplorer.createDirectory(target);"))
            {
            ...
            var upload = new UICContextMenuItem($"#{fileExplorer.GetId()}:has(.can-create-file)", new UICDropdownItem(TranslatableSaver.Save("FileExplorer.Upload")) { Icon = IconDefaults.Upload?.Invoke() }, new UICCustom("uic.fileExplorer.uploadPage(target);"))
            {
